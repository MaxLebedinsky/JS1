...
function amountButtonHandler(product, operation) {
    switch (operation) {
        case 'plus':
            product.amount++;
            break;
...
function renderBasket(basket) {
    ...
    basketHeader.classList.add('accordeon-header');
    ...
    for (let i = 0; i < basket.items.length; i++) { // формируем и выводим строки с товарами в корзине
        ...
        basketLine.innerHTML = `Название: <b>${basket.items[i].name}</b> Кол-во:  <button class="minus${i}"> - </button> <b>${basket.items[i].amount}</b> <button class="plus${i}"> + </button>   Цена: <b>$${basket.items[i].price}</b>   Скидка: <b>${basket.items[i].discount}%</b>   Стоимость: <b>$${basket.items[i].subtotal}</b>`; // кнопки плюс и минус с уникальными классами генерируются внутри строки

        basketContainer.appendChild(basketLine);

        let btnMinus = document.querySelector('.minus' + i);
        btnMinus.addEventListener('click', function () {
            amountButtonHandler(basket.items[i], 'minus'); // уменьшаем кол-во товара в строке корзины
            if (basket.items[i].amount == 0) { // если достигли 0, то удаляем этустроку корзины
                basket.items.splice(i, 1);
                renderBasket(basket);
            }
        });

        let btnPlus = document.querySelector('.plus' + i);
        btnPlus.addEventListener('click', function () { // увеличиваем кол-во товара в строке корзины
            amountButtonHandler(basket.items[i], 'plus');
        });
    ...
    continueButton.innerText = 'Далее';
    basketContainer.appendChild(continueButton);
};
...
document.addEventListener('click', (event) => {
    ...
    if (event.target.classList.contains('accordeon-header')) {
        let hasHidden = parent.classList.contains('hidden-block');
        parent.classList.toggle('hidden-block');
    }

    if (event.target.classList.contains('continue-btn')) {
        parent.classList.add('hidden-block');
        let nextElement = parent.nextElementSibling;
        nextElement.classList.remove('hidden-block');
    }
